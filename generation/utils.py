...
class GenerationMixin:
    ...
    def _prepare_model_inputs(
        ...
        """
        print(f'self.main_input_name: {self.main_input_name}')
    ...
    def _update_model_kwargs_for_generation(
        ...
        logger.info(f'is_encoder_decoder: {is_encoder_decoder}')
        logger.info(f' model_kwargs["attention_mask"] shape: { model_kwargs["attention_mask"].shape}')

        if not is_encoder_decoder:
            # update attention mask
            if "attention_mask" in model_kwargs:
                attention_mask = model_kwargs["attention_mask"]
                logger.info(f'attention_mask.new_ones((attention_mask.shape[0], 1)): {attention_mask.new_ones((attention_mask.shape[0], 1))}')
                model_kwargs["attention_mask"] = torch.cat(
                    [attention_mask, attention_mask.new_ones((attention_mask.shape[0], 1))], dim=-1
                )
                logger.info(f'model_kwargs["attention_mask"]: {model_kwargs["attention_mask"]}')
    ...
    def compute_transition_scores(
        ...
        >>> # Example 1: logging.info the scores for each token generated with Greedy Search
        ...
        ...     logging.info(f"| {tok:5d} | {tokenizer.decode(tok):8s} | {score.numpy():.3f} | {np.exp(score.numpy()):.2%}")
        |   262 |  the     | -1.414 | 24.33%
        |  1110 |  day     | -2.609 | 7.36%
        ...
        >>> logging.info(np.allclose(outputs.sequences_scores, reconstructed_scores))
        True
        ```"""
    ...
    def generate(
        ...
        **kwargs,
        ...
        logger.info(f'enter generate in util')
        ...
        if generation_config is None:
            ...
            if self.generation_config._from_model_config and self.generation_config._original_object_hash == hash(
                ...
                print(f'generate new config: {new_generation_config}')
                print(f'self.config: {self.config}')
                if new_generation_config != self.generation_config:
                    print('new_generation_config != self.generation_config:')
                    warnings.warn(
        ...
        print(f'inputs_tensor; {inputs_tensor},  model_input_name:{model_input_name}, model_kwargs:{model_kwargs}')
        ...
        logger.info(f'self.config.is_encoder_decoder : {self.config.is_encoder_decoder}')
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        ```"""
        logger.info(f'input_ids in greedy search: {input_ids} shape: {input_ids.shape}')
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        ...
        loop_cnt = 0
        while True:
            logger.info(f'generating {loop_cnt} token.................')
            loop_cnt += 1
            print(f'#'*100)
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            logger.info(f'============model_inputs["input_ids"] : {model_inputs["input_ids"]}')
            ...
            if synced_gpus and this_peer_finished:
                ...
                continue  # don't waste resources running the code we don't need
            logger.info(f'outputs shape: {outputs.logits.shape}')
            # get last element in dimension 1 of torch.Size([1, 7, 4096])
            next_token_logits = outputs.logits[:, -1, :]
            logger.info(f'next_token_logits shape: {next_token_logits.shape}')

            logger.info(f' input_ids shape: {input_ids.shape}')

            logger.info(f'logits_processor: {logits_processor}')
            # pre-process distribution
            next_tokens_scores = logits_processor(input_ids, next_token_logits)
            logger.info(f'next_tokens_scores: {next_tokens_scores} , shape : {next_tokens_scores.shape}')
            logger.info(f'output_scores: {output_scores}')
            logger.info(f'output_attentions: {output_attentions}')
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            logger.info(f'catted input_ids shape: {input_ids.shape}')
            ...
            logger.info(f'this_peer_finished: {this_peer_finished}')
            # if eos_token was found in one sentence, set sentence to finished
            if eos_token_id_tensor is not None:
                unfinished_sequences = unfinished_sequences.mul(
                    next_tokens.tile(eos_token_id_tensor.shape[0], 1).ne(eos_token_id_tensor.unsqueeze(1)).prod(dim=0)
        ...
        if return_dict_in_generate:
            logger.info(f'return_dict_in_generate: true')
            if self.config.is_encoder_decoder:
                logger.info(f'self.config.is_encoder_decoder: true')
            ...
            logger.info('return_dict_in_generate: not true')
            return input_ids
        ...
        **model_kwargs,
        ...
        while True:
            if synced_gpus:
                ...
                if this_peer_finished_flag.item() == 0.0:
                    break
            logger.info(f'generating {generate_cnt} token.................')
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
